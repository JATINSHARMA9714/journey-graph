// Main component for adding/editing journeys with tabs
...
// Tab configuration
const tabs = [
  { id: 'create', label: 'Journey Info', component: CreateJourneyTab },
...
// Main content component (needs to be inside JourneyProvider)
function AddJourneyFlowContent({ onBack, isPreviewOnly = false }: { onBack: () => void; isPreviewOnly?: boolean }) {
  const { journey, saveCurrentJourney, toggleJourneyActive } = useJourney();
  const [activeTab, setActiveTab] = useState(isPreviewOnly ? 6 : 0); // Start at preview if preview-only
  ...
  // Handle activating/deactivating journey
...
export default function AddJourneyFlow({ onBack, initialJourney, isPreviewOnly }: AddJourneyFlowProps) {
  return (
